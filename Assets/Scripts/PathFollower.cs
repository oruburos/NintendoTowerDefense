...
public class PathFollower : MonoBehaviour
{
	...
	void OnEnable ()
    {
        switch( Type )
        {
            case PathType.Ground:
                {
                    path = GameObject.Find("WalkingPath").GetComponent<PathCreator>().path;
                    break;
                }
            case PathType.Air:
                {
                    path = GameObject.Find("FlyingPath").GetComponent<PathCreator>().path;
                    break;
                }
        }
	...
	void FixedUpdate ()
    {
        ...
        transform.eulerAngles = new Vector3(0.0f, 0.0f, 1f);
